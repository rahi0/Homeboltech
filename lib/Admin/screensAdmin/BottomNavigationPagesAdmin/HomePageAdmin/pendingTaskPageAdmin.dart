...
class PendingTaskAdminPage extends StatefulWidget {
  @override
  _PendingTaskAdminPageState createState() => _PendingTaskAdminPageState();
}

class _PendingTaskAdminPageState extends State<PendingTaskAdminPage> {
  bool isLoading = true;
  bool noData = false;
...
ScrollController _controller = new ScrollController();
      ...
      if (_controller.position.atEdge) {
        ...
        // you are at top position
          ...
          if(isSearching){
            ...
            if(_isLoadingMore == false){
              _loadMorePendingTask(_lastId); //api will be call at the bottom at the list
            }else{
              print('loading data');
            }
          }
  ...
  Future<void> _getPendingTaskData() async {
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&status=Pending');
    ...
    //   else
      ...
      pendingTaskRes = adminPendingTaskModelFromJson(res.body);

       store.dispatch(AdminPendingTaskAction(pendingTaskRes.data)); // update data in store...
       
       //print(store.state.biographicalInfoState);
       if (!mounted) return;
       setState(() {
         pendingTaskData = store.state.adminPendingTaskState;
       });

       if (pendingTaskData.length > 0) {
        _lastId = pendingTaskData[pendingTaskData.length - 1].id;
      }
       print("pendingTaskData......................");
       print(pendingTaskData[0].status);
...
//       });
  ...
  //////////////// get more task start ////////////////
  Future<void> _loadMorePendingTask(lastID) async {
    setState(() {
      _isLoadingMore = true;
    });

    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&status=Pending&lastId=$lastID');
    final body = json.decode(res.body);
    ...
    if (res.statusCode == 200) {
      ...
      for (int i = 0; i < loadList.length; i++) {
        store.state.adminPendingTaskState.add(loadList[i]);
      }
    }
  ...
  //////////////// search task start ////////////////
  Future<void> _searchPendingTask(lastID) async {
    setState(() {
      isLoading = true;
    });
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&status=Pending&str=${searchController.text}');
    final body = json.decode(res.body);
  ...
  Future<void> _loadMoreSearchTask() async {
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&status=Pending&str=${searchController.text}&lastId=$_searchlastId');
    final body = json.decode(res.body);
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: TextField(
                      ...
                      onSubmitted: _searchPendingTask,
                      onChanged: (value){
                        if(value == ""){
                          setState(() {
                            isSearching = false;
                            ...
                            children: List.generate(
                              ...
                              return GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: Color.fromRGBO(236, 122, 96, 0.1),
                    ...
                    child: Column(
                      children: [
                        Container(
                          width: MediaQuery.of(context).size.width,
                          child: Wrap(
                            alignment: pendingTaskData.length <=1 ? WrapAlignment.start : WrapAlignment.spaceAround,
                           // spacing: 10.0,
                            children: List.generate(
                              pendingTaskData.length , (index) {
                          ...
                          // }
                              return GestureDetector(
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    ScaleRoute(page: TaskDetailsAdminPage(pendingTaskData[index])),
                                  );
                                },
                                child: Container(
                                  margin: EdgeInsets.only(bottom: 10),
                                  ...
                                  children: [
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Container(
                                            margin: EdgeInsets.only(left: 15),
                                            child: Text(
                                              pendingTaskData[index].createdAt == null ? "" :
                                              "${DateFormat.yMMMMd().format(pendingTaskData[index].createdAt)}", //start date oito
                                              //'Sep 20, 2020',
                                              style: TextStyle(
                                                fontSize: 12,
                                                color: Color.fromRGBO(45, 45, 45, 0.8),
                                          ...
                                          onSelected: (selected) {
                                           if(selected == 1){
                                             removeTaskButton(pendingTaskData[index]);
                                           }
                                          },
                                          shape: RoundedRectangleBorder(
                                              borderRadius: BorderRadius.all(
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        children: [
                                          Text(
                                            'by ${pendingTaskData[index].user.firstname}',
                                            style: TextStyle(
                                              fontSize: 12,
                                              color: Color.fromRGBO(45, 45, 45, 0.7),
                                            ),
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        children: [
                                          Text(
                                            'In ${pendingTaskData[index].status}',
                                            style: TextStyle(
                                              fontSize: 12,
                                              fontWeight: FontWeight.w500,
                                              color: Color.fromRGBO(45, 45, 45, 1),
                                              ...
                                              children: [
                                                Container(
                                                  ...
                                                  child: CircleAvatar(
                                                    radius: 18,
                                                    backgroundImage: 
                                                    pendingTaskData[index].tech.image != null ?
                                                    NetworkImage(
                                                        "${pendingTaskData[index].tech.image}")
                                                    :
                                                     AssetImage(
                                                      'assets/images/person.jpg',
                                                    ),
                          ...
                          //   controller: _controller,
                          //   shrinkWrap: true,
                          //   itemCount: currentTaskData.length,
                          ...
                          //    );
                          // }
                          //     // return currentTaskData.length - 1 == index
                          //     //                 ? _isLoadingMore
                          //     //                     ? Container(
                          //     //                         width: MediaQuery.of(context)
                          //     //                             .size
                  ...
                  // currentTaskData.length - 1 == index
                  //                       ?
                                         _isLoadingMore
                                            ? Container(
                                                width: MediaQuery.of(context)
     ...
     isLoading = true;
      ...
      var data = {
       ...
       if(res.statusCode == 402){
        ...
        for(var d in store.state.adminPendingTaskState){
        if(d.id == taskRemove.id){
          store.state.adminPendingTaskState.remove(taskRemove);
          store.dispatch(AdminPendingTaskAction(store.state.adminPendingTaskState));
          break;
        }else{
