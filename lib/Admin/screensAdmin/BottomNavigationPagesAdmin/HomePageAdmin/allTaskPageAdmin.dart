...
class AllTaskAdminPage extends StatefulWidget {
  @override
  _AllTaskAdminPageState createState() => _AllTaskAdminPageState(); 
}

class _AllTaskAdminPageState extends State<AllTaskAdminPage> {
  bool isLoading = true;
      ...
      if (_controller.position.atEdge) {
        ...
        // you are at top position
          ...
          if(isSearching){
            ...
            if(_isLoadingMore == false){
              _loadMoreAllTask(_lastId); //api will be call at the bottom at the list
            }else{
              print('loading data');
            }
          }
  ...
  Future<void> _getallTaskData() async {
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '');
    ...
    print(body);
     ...
     if (res.statusCode != 200) {
      ...
      allTaskRes = adminPendingTaskModelFromJson(res.body);

       store.dispatch(AdminAllTaskAction(allTaskRes.data)); // update data in store...
       
       //print(store.state.biographicalInfoState);
       if (!mounted) return;
       setState(() {
         allTaskData = store.state.adminAllTaskState;
       });

       if (allTaskData.length > 0) {
        _lastId = allTaskData[allTaskData.length - 1].id;
      }
       print("_lastId......................");
       print(_lastId);
...
//       });
  ...
  //////////////// get more task start ///////////////
  Future<void> _loadMoreAllTask(lastID) async {
    setState(() {
      _isLoadingMore = true;
    });

    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&lastId=$lastID');
    final body = json.decode(res.body);
    ...
    if (res.statusCode == 200) {
      ...
      for (int i = 0; i < loadList.length; i++) {
        store.state.adminAllTaskState.add(loadList[i]);
      }
    }
  ...
  //////////////// search task start ////////////////
  Future<void> _searchAllTask(lastID) async {
    setState(() {
      isLoading = true;
    });
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&str=${searchController.text}');
    final body = json.decode(res.body);
  ...
  Future<void> _loadMoreSearchTask() async {
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&str=${searchController.text}&lastId=$_searchlastId');
    final body = json.decode(res.body);
                            ...
                            children: List.generate(
                              ...
                              return GestureDetector(
                                ...
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: store.state.adminSearchTaskState[index].status == 'Pending' ?
                                    Color.fromRGBO(236, 122, 96, 0.1) :
                                    store.state.adminSearchTaskState[index].status == 'Processing' ?
                    ...
                    child: Column(
                      children: [
                        Container(
                          width: MediaQuery.of(context).size.width,
                          child: Wrap(
                            alignment: allTaskData.length <=1 ? WrapAlignment.start : WrapAlignment.spaceAround,
                           // spacing: 10.0,
                            children: List.generate(
                              allTaskData.length , (index) {
                          ...
                          // }
                              return GestureDetector(
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    ScaleRoute(page: TaskDetailsAdminPage(allTaskData[index])),
                                  );
                                },
                                child: Container(
                                  margin: EdgeInsets.only(bottom: 10),
                                  ...
                                  width: MediaQuery.of(context).size.width / 2.4,
                                  decoration: BoxDecoration(
                                    color: allTaskData[index].status == 'Pending' ?
                                    Color.fromRGBO(236, 122, 96, 0.1) :
                                    allTaskData[index].status == 'Processing' ?
                                    Color.fromRGBO(1, 1, 211, 0.1) :
                                    Color.fromRGBO(3, 172, 98, 0.1),
                                    borderRadius: BorderRadius.circular(10),
                                  ),
                                  ...
                                  children: [
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Container(
                                            margin: EdgeInsets.only(left: 15),
                                            child: Text(
                                              allTaskData[index].createdAt == null ? "" :
                                              "${DateFormat.yMMMMd().format(allTaskData[index].createdAt)}", //start date oito
                                              //'Sep 20, 2020',
                                              style: TextStyle(
                                                fontSize: 12,
                                                color: Color.fromRGBO(45, 45, 45, 0.8),
                                          ...
                                          onSelected: (selected) {
                                           if(selected == 1){
                                             removeTaskButton( allTaskData[index]);
                                           }
                                          },
                                          shape: RoundedRectangleBorder(
                                              borderRadius: BorderRadius.all(
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        children: [
                                          Text(
                                            'by ${allTaskData[index].user.firstname}',
                                            style: TextStyle(
                                              fontSize: 12,
                                              color: Color.fromRGBO(45, 45, 45, 0.7),
                                            ),
                                              ...
                                              children: [
                                                Container(
                                                  ...
                                                  child: CircleAvatar(
                                                    radius: 18,
                                                    backgroundImage: 
                                                    allTaskData[index].tech.image != null ?
                                                    NetworkImage(
                                                        "${allTaskData[index].tech.image}")
                                                    :
                                                     AssetImage(
                                                      'assets/images/person.jpg',
                                                    ),
     ...
     isLoading = true;
      ...
      var data = {
       ...
       if(res.statusCode == 402){
        ...
        for(var d in store.state.adminAllTaskState){
        if(d.id == taskRemove.id){
          store.state.adminAllTaskState.remove(taskRemove);
          store.dispatch(AdminAllTaskAction(store.state.adminAllTaskState));
          break;
        }else{
          print("not deleted from all");
        }
      }
