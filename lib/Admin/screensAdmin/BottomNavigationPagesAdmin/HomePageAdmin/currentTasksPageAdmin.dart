...
class CurrentTasksAdminPage extends StatefulWidget {
  @override
  _CurrentTasksAdminPageState createState() => _CurrentTasksAdminPageState(); 
}

class _CurrentTasksAdminPageState extends State<CurrentTasksAdminPage> {
  bool isLoading = true;
 ...
 Future<void> _getcurrentTaskData() async {
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&status=Processing');
    ...
    print(body);
     ...
     if (res.statusCode != 200) {
      ...
      currentTaskRes = adminPendingTaskModelFromJson(res.body);

       store.dispatch(AdminCurrentTaskAction(currentTaskRes.data)); // update data in store...
       
       //print(store.state.biographicalInfoState);
       if (!mounted) return;
       setState(() {
         currentTaskData = store.state.adminCurrentTaskState;
       });

       if (currentTaskData.length > 0) {
        _lastId = currentTaskData[currentTaskData.length - 1].id;
      }
       print("_lastId......................");
       print(_lastId);
...
//       });
  ...
  Future<void> _loadMoreCurrentTask(lastID) async {
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&status=Processing&lastId=$lastID');
    final body = json.decode(res.body);
    ...
    if (res.statusCode == 200) {
      ...
      for (int i = 0; i < loadList.length; i++) {
        store.state.adminCurrentTaskState.add(loadList[i]);
      }
    }
  ...
  Future<void> _searchCurrentTask(lastID) async {
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&status=Processing&str=${searchController.text}');
    final body = json.decode(res.body);
  ...
  Future<void> _loadMoreSearchTask() async {
    ...
    var res = await CallApi().getDataWithTokenNPercent('sadmin/all_task_tech', '&status=Processing&str=${searchController.text}&lastId=$_searchlastId');
    final body = json.decode(res.body);
                    ...
                    child: Column(
                      children: [
                        Container(
                          width: MediaQuery.of(context).size.width,
                          child: Wrap(
                            alignment: currentTaskData.length <=1 ? WrapAlignment.start : WrapAlignment.spaceAround,
                           // spacing: 10.0,
                            children: List.generate(
                              currentTaskData.length , (index) {
                          ...
                          // }
                              return GestureDetector(
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    ScaleRoute(page: TaskDetailsAdminPage(currentTaskData[index])),
                                  );
                                },
                                child: Container(
                                  margin: EdgeInsets.only(bottom: 10),
                                  ...
                                  children: [
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Container(
                                            margin: EdgeInsets.only(left: 15),
                                            child: Text(
                                              currentTaskData[index].createdAt == null ? "" :
                                              "${DateFormat.yMMMMd().format(currentTaskData[index].createdAt)}", //start date oito
                                              //'Sep 20, 2020',
                                              style: TextStyle(
                                                fontSize: 12,
                                                color: Color.fromRGBO(45, 45, 45, 0.8),
                                          ...
                                          onSelected: (selected) {
                                           if(selected == 1){
                                             removeTaskButton( currentTaskData[index]);
                                           }
                                          },
                                          shape: RoundedRectangleBorder(
                                              borderRadius: BorderRadius.all(
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        children: [
                                          Text(
                                            'by ${currentTaskData[index].user.firstname}',
                                            style: TextStyle(
                                              fontSize: 12,
                                              color: Color.fromRGBO(45, 45, 45, 0.7),
                                            ),
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        children: [
                                          Text(
                                            'In ${currentTaskData[index].status}',
                                            style: TextStyle(
                                              fontSize: 12,
                                              fontWeight: FontWeight.w500,
                                              color: Color.fromRGBO(45, 45, 45, 1),
                                              ...
                                              children: [
                                                Container(
                                                  ...
                                                  child: CircleAvatar(
                                                    radius: 18,
                                                    backgroundImage: 
                                                    currentTaskData[index].tech.image != null ?
                                                    NetworkImage(
                                                        "${currentTaskData[index].tech.image}")
                                                    :
                                                     AssetImage(
                                                      'assets/images/person.jpg',
                                                    ),
     ...
     isLoading = true;
      ...
      var data = {
       ...
       if(res.statusCode == 402){
        ...
        for(var d in store.state.adminCurrentTaskState){
        if(d.id == taskRemove.id){
          store.state.adminCurrentTaskState.remove(taskRemove);
          store.dispatch(AdminCurrentTaskAction(store.state.adminCurrentTaskState));
          break;
        }else{
